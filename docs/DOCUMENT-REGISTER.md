...
| [015](00-planning/decisions/ADR-015-success-criteria-and-priorities.md) FR-R08 vs the strongest baseline; both action spaces; staged compute; no frames published; S06 deadline | **Proposed** | Guide | Webster is not significantly better than fixed-time (p=0.225). **Closes P10 and P11 pending sign-off** |
| [016](00-planning/decisions/ADR-016-two-phase-corpus.md) Two-phase corpus: build the pipeline on Bellevue's 101 h of pole-mounted intersection video, claim on India | **Proposed** | Guide | The blocker was never Indian data — it is 360 s of continuous fixed-camera video (A15). UA-DETRAC's 24–40 s clips are 9× too short; BMD-45 is stills. Bellevue is ~101 h at 5 fixed intersections |
| [017](00-planning/decisions/ADR-017-pcu-thresholds.md) Congestion thresholds in Indo-HCM PCU, calibrated per camera and pre-registered | **Proposed** | Guide | P20 measured §14.1's `LOW < 5` never firing on the only camera we have. Raw counts took 24 distinct values to PCU's 139, so 22% of samples sat exactly on the calibration thresholds and balanced classes were unreachable. Separately: the naive baseline moves 69.0% → 31.0% on identical footage purely from threshold placement, so thresholds must be fixed before any model trains |
| [018](00-planning/decisions/ADR-018-itd-distillation.md) Distil ITD-x into our YOLOv8s by pseudo-labelling deployment footage | **Proposed** | Team | Weight transfer x→s is impossible (0.0% of our parameters have a shape-compatible counterpart), so the fix is distillation, not fine-tuning. Piloting found the teacher hallucinates large vehicles — truck box median 13% of frame against car at 0.7% — and has no e_rickshaw/cattle. Four acceptance criteria pre-registered on real labelled test data |
